...
function scoreShipment(shipment: Shipment): ScoredShipment {
  ...
  const recallBatches = new Set(getRecalls().filter((r) => r.status === "OPEN").map((r) => r.batchNumber));
  const custodyGaps = getCustodyGaps(shipment.id);
  const unauthorizedIds = new Set(ds.tradingPartners.filter((p) => p.auth === "UNAUTHORIZED").map((p) => p.id));
  const involvesUnauthorized = ds.custodyEvents.some(
    (c) => c.shipmentId === shipment.id && (unauthorizedIds.has(c.fromPartyId) || unauthorizedIds.has(c.toPartyId)),
  );
  ...
  let delay = 8;
  if (shipment.primaryMode === "OCEAN") { delay += 18; delayDrivers.push({ factor: "Ocean freight transit", weight: 18 }); }
  else if (shipment.primaryMode === "AIR") { delay += 5; delayDrivers.push({ factor: "Air freight", weight: 5 }); }
  if (atCustoms) { delay += 30; delayDrivers.push({ factor: "Currently held in customs", weight: 30 }); }
  else if (shipment.status === "DELAYED") { delay += 26; delayDrivers.push({ factor: "In delayed status", weight: 26 }); }
  ...
  if (shipment.hasExcursion) { delay += 12; delayDrivers.push({ factor: "Active disruption en route", weight: 12 }); }
  if (recallBatches.has(shipment.batchNumber)) { delay += 9; delayDrivers.push({ factor: "Recall-impacted consignment", weight: 9 }); }
  if (ds.shipmentEvents.some((e) => e.shipmentId === shipment.id && e.eventType === "CUSTOMS_ENTRY")) {
    delay += 8;
  ...
  if (isCold && shipment.delayHours > 12) { exc += 16; excDrivers.push({ factor: "Extended dwell > 12h", weight: 16 }); }
  if (shipment.hasExcursion) { exc += 34; excDrivers.push({ factor: "Active excursion detected", weight: 34 }); }
  ...
  let recall = 4;
  if (recallBatches.has(shipment.batchNumber)) { recall += 55; recallDrivers.push({ factor: "Batch under active recall", weight: 55 }); }
  if (shipment.hasExcursion) { recall += 20; recallDrivers.push({ factor: "Temperature excursion", weight: 20 }); }
  if (isCold) { recall += 8; recallDrivers.push({ factor: "Temperature-sensitive product", weight: 8 }); }
  if (shipment.packageCount > 1500) { recall += 8; recallDrivers.push({ factor: "High package volume", weight: 8 }); }

  // --- Traceability-failure risk ---
  const traceDrivers: Driver[] = [];
  let trace = 6;
  if (!shipment.traceabilityComplete) { trace += 45; traceDrivers.push({ factor: "Incomplete custody chain", weight: 45 }); }
  ...
  if (involvesUnauthorized) { trace += 30; traceDrivers.push({ factor: "Unauthorized partner interaction", weight: 30 }); }
...
function scorePartners(): PartnerRiskScore[] {
  ...
  return getPartners()
    .map((p) => {
      ...
      if (p.auth === "UNAUTHORIZED") drivers.push({ factor: "Not an authorized trading partner", weight: 40 });
      if (p.license === "EXPIRED") drivers.push({ factor: "Expired licence", weight: 30 });
      if (p.license === "EXPIRING_SOON") drivers.push({ factor: "Licence expiring soon", weight: 12 });
      const partnerGaps = ds.custodyEvents.filter(
        (c) => !c.valid && (c.fromPartyId === p.id || c.toPartyId === p.id),
      ).length;
      if (partnerGaps > 0) drivers.push({ factor: `${partnerGaps} custody-doc gap(s)`, weight: partnerGaps * 8 });
      if (p.riskScore >= 60) drivers.push({ factor: `Elevated base risk score (${p.riskScore})`, weight: 20 });
...
function buildAlerts(scored: ScoredShipment[], partnerScores: PartnerRiskScore[]): PredictiveAlert[] {
  const alerts: PredictiveAlert[] = [];
  const byDelay = [...scored].sort((a, b) => b.score.delayProbability - a.score.delayProbability);
  const byExc = [...scored].sort((a, b) => b.score.excursionProbability - a.score.excursionProbability);
  const byTrace = [...scored].sort((a, b) => b.score.traceabilityFailureRisk - a.score.traceabilityFailureRisk);
  ...
  const risingPartner =
    partnerScores.find((p) => p.name === "ABC Logistics") ?? partnerScores.find((p) => p.trend === "RISING");
  if (risingPartner) {
    alerts.push({
...
export function getPredictiveBundle(): PredictiveBundle {
  ...
  const scores = scored
    .map((s) => s.score)
    .sort((a, b) => {
      const am = Math.max(a.delayProbability, a.excursionProbability, a.recallExposure, a.traceabilityFailureRisk);
      const bm = Math.max(b.delayProbability, b.excursionProbability, b.recallExposure, b.traceabilityFailureRisk);
      return bm - am;
    });
